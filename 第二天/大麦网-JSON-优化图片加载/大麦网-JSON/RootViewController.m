...
-(NSMutableArray *)scrollImages
{
    if (_scrollImages == nil) {
        _scrollImages = [[NSMutableArray alloc] init];
    }
...
//    _tableConnection = [[NSURLConnection alloc] initWithRequest:tableReqest delegate:self];
    ASIHTTPRequest * requestTable = [[ASIHTTPRequest alloc] initWithURL:tableURL];
    requestTable.tag = 2;
    requestTable.delegate = self;
    [requestTable startSynchronous];
...
-(void)requestFinished:(ASIHTTPRequest *)request
{
    if (request.tag == 1) {
        ...
        for (NSDictionary * dict in imagesArray) {
            ...
            [self.scrollImages addObject:imageURL];
        }
        
        self.scrollView.contentSize = CGSizeMake(self.scrollImages.count * 320, 0);
        for (int i = 0; i < self.scrollImages.count; i ++) {
            UIImageView * imageView = [[UIImageView alloc] init];
            imageView.frame = CGRectMake(320 * i, 0 , 320, 100);
        ...
        for (int i = 0; i < self.scrollImages.count; i ++) {
            ASIHTTPRequest * requset = [ASIHTTPRequest requestWithURL:[self.scrollImages objectAtIndex:i]];
